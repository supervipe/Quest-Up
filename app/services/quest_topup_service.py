...
from app.core.constants import NPCOfferStatus, QuestSource, QuestStatus, WeeklyQuestStatus
...
class QuestTopUpService:
    ...
    async def open_session(self, db: AsyncSession, user: User, lat: float | None, lng: float | None, timezone: str | None) -> dict:
        ...
        weekly = await db.scalar(select(WeeklyCommunityQuest).where(WeeklyCommunityQuest.status == WeeklyQuestStatus.active).order_by(WeeklyCommunityQuest.starts_at.desc()))
        ...
        await db.commit()
        return {"normal": normal, "weekly": weekly, "npc_offer": npc_offer}
